...
**
** GENERATED by tools/regen-computedgoto.py from ring_vm_execute()'s
** switch: one label per opcode, bodies identical, label table in
** codegen.h enum order (opcodes with no case there fall through, as
** the switch does). The win in wasm is not the goto itself (clang
** lowers both this and a dense switch to br_table) — it is that
** fetch, dispatch and the stack check live in ONE loop with no
** function call per instruction. Behavior is held identical to the
** fetch loop by the oracle battery (~850 programs byte-exact vs
** native ring.exe).
**
** If codegen.h's opcode enum or a case body changes, RERUN THE
** GENERATOR — a stale label table dispatches the wrong opcode.
** Enabled with -DRING_VM_COMPUTEDGOTO (build.zig).
*/
